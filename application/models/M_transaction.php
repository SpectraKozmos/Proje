...
class M_transaction extends CI_Model {
    ...
    {
        return $this->db->select('transaction.*, book.book_title, book.price, user.fullname')
                        ->from('transaction')
                        ->join('transaction_detail', 'transaction_detail.transaction_code = transaction.transaction_code')
                        ->join('book', 'book.book_code = transaction_detail.book_code')
                        ->join('user', 'user.user_code = transaction.user_code')
                        ->order_by('transaction.transaction_code', 'DESC')
                        ->get()
                        ->result();
    }
